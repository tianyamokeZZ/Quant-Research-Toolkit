...
def fitness_func(ga_instance, solution, solution_idx):
    ...
    用“多场景 + WFE 惩罚”的方式作为 GA 的 fitness（越大越好）。
    """
    # 先把解修剪成“合法”的：
    ...
    ret_list = []
    dd_list = []
    scenario_scores = []  # 👈 NEW: 专门存 combo 目标下每个场景的 score
    ...
    for scen in scenario_list:
        ...
        if GA_OBJECTIVE == "rmse":
            ...
            continue  # 这个场景结束，继续下一个

        # ========= 2️⃣ 回测类目标：train + test 都跑一遍 =========

        # --- 训练集预测 & 回测 ---
        y_pred_train = model.predict(X_tr)
        close_train = merged.loc[train_mask_s, "eth_hist_c"].reset_index(drop=True)
        time_train  = merged.loc[train_mask_s, "datetime"].reset_index(drop=True)

        bt_tr = run_backtest_simple(
            y_pred_train,
            close_train,
            time_train,
        )
        ...
        # --- 测试集预测 & 回测 ---
        y_pred_test = model.predict(X_te)
        bt_te = run_backtest_simple(
            y_pred_test,
            scen["close_test"],
            scen["time_test"],
        )

        sh_tr = float(bt_tr["sharpe_annual"])
        sh_te = float(bt_te["sharpe_annual"])
        ret_te = float(bt_te["total_return_pct"])
        dd_te  = float(bt_te["max_drawdown_pct"])

        sharpe_list.append(sh_te)
        ret_list.append(ret_te)
        dd_list.append(dd_te)

        # 如果不是 combo 目标，先不算 WFE，后面用平均 sharpe/收益/回撤来评分
        if GA_OBJECTIVE != "combo":
            continue

        # ========= 3️⃣ combo 目标下：引入 WFE 惩罚 =========
        # 基础分数：只看测试集表现（和你原来的写法一致）
        w_sharpe = 1.0   # 夏普权重
        ...
        # ---- WFE 惩罚：希望 test ≈ 0.4~1.0 × train，负的或过大都要惩罚 ----
        if sh_tr <= 0 or sh_te <= 0:
            # 训练集或测试集 Sharpe 都不行 → 认为这个场景本身就很拉胯
            wfe_penalty = 0.2
        else:
            wfe = sh_te / (sh_tr + 1e-9)  # Walk-Forward Efficiency (Sharpe)

            # 目标区间大致是 [0.4, 1.0]，0.6 落在中间
            target_low  = 0.4
            target_high = 0.8
            ...
            if wfe < 0:
                ...
                # 0 ~ 0.4 之间：线性从 0 → 1
                wfe_penalty = wfe / target_low
            elif wfe <= target_high:
                # 0.4 ~ 1.0：认为是“正常波动”，不惩罚
                wfe_penalty = 1.0
            else:
                # >1.0：测试集比训练集还好不少，越夸张越惩罚
                # wfe = 1.0 时 penalty=1；wfe>=2 时 penalty<=0
                wfe_penalty = max(0.0, 1.0 - (wfe - target_high))

        scenario_score = base_score * wfe_penalty
        scenario_scores.append(scenario_score)

    # ========= 4️⃣ 汇总成一个 GA 分数 =========

    # RMSE 目标：平均 RMSE 越小越好 → 取负
    if GA_OBJECTIVE == "rmse":
        if not rmse_list:
            return -1e9
        return -float(np.mean(rmse_list))

    # 回测类：如果一个场景都没算成，说明这条染色体太拉，直接差评
    if not sharpe_list and not ret_list and not dd_list:
        return -1e9

    # --- combo：用“场景 score 的平均”作为最终适应度 ---
    if GA_OBJECTIVE == "combo":
